...
module.exports = {
    ...
    compiler: function(id, tokens, pathToTwig) {
        var includes = [];
        var resourcePath = mapcache.get(id);
        var processDependency = function(token) {
            if (_base) {
                includes.push(path.join(_base, token.value));
                token.value = hashGenerator(path.resolve(_base, token.value));
            }
            else {
        ...
        var renderWith = _base ? "Object.assign({__templatePath: '" + resourcePath.replace(_base + '/', '') + "'}, context)" : 'context';
        output.push('module.exports = function(context) { return template.render(' + renderWith + '); }');
